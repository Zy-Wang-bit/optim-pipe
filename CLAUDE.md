...
管理节点（跳板机）：`121.192.179.122`，内网 IP 段 `10.10.10.x`。

### 分区与可用节点（未列出的节点不可用）

| 分区 | 节点 | GPU | CPU/节点 | 内存/节点 | 默认内存/CPU | 备注 |
|------|------|-----|----------|-----------|-------------|------|
| A40 | gpu[5-9] | 4×A40 | 32 | 503 GB | 16 GB | |
| RTX3090* | gpu[14-17] | 4×RTX3090 | 24 | 251 GB | 12 GB | |
| AMD | computer[1-4] | 无 | 128 | 1007 GB | 8 GB | 纯 CPU 计算 |
...
### 新对话启动
1. 如果用户明确指定了要恢复的任务（如"继续做 XXX"），只读取该任务的文档
2. 如果用户没有指定任务，正常响应用户请求，不主动扫描 `.tasks/active/`
3. 恢复任务时读取其 `plan.md` 和 `progress.md`，用"五问重启检查"恢复上下文

### Session 内任务隔离
- 一个 session 只处理一个任务（或用户的即时请求）
- 不得在执行过程中主动读取或合并 `.tasks/active/` 下的其他任务
- 如果用户在同一 session 中提出与当前任务无关的新的大型任务，应作为独立任务创建，不与当前任务融合
