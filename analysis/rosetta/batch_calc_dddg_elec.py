...
def process_single_pdb(args):
    ...
    Args:
        args: (pdb_path, binder_chains, target_chains, verbose)
    
    Returns:
        dict: 结果字典
    """
    pdb_path, binder_chains, target_chains, verbose = args
...
def batch_calculate_dddg_elec(pdb_dir, output_csv=None, n_jobs=1, verbose=False):
    """
    ...
    Args:
        ...
        n_jobs: 并行进程数（默认1，单进程；-1表示使用所有CPU核心）
        verbose: 是否打印详细信息
    """
    # 查找所有PDB文件
    ...
    n_jobs = min(n_jobs, len(pdb_files))  # 不超过文件数
    ...
    binder_chains = ["A"]
    target_chains = ["B"]
    args_list = [(pdb_path, binder_chains, target_chains, verbose) 
    ...
    if n_jobs == 1:
        ...
        for i, args in enumerate(args_list, 1):
            pdb_name = os.path.basename(args[0])
            print(f"[{i}/{len(pdb_files)}] 处理 {pdb_name}...", end=" ")
            row = process_single_pdb(args)
            if row["dddG_elec"] is not None:
                print(f"✓ dddG_elec={row['dddG_elec']:.3f}")
            else:
                print(f"✗ {row['status']}")
            results.append(row)
    else:
        ...
        with Pool(processes=n_jobs) as pool:
            # 使用imap_unordered来获取实时进度
            results = []
            for i, row in enumerate(pool.imap_unordered(process_single_pdb, args_list), 1):
                pdb_name = row["pdb_name"]
                if row["dddG_elec"] is not None:
                    print(f"[{i}/{len(pdb_files)}] {pdb_name}: ✓ dddG_elec={row['dddG_elec']:.3f}")
                else:
                    print(f"[{i}/{len(pdb_files)}] {pdb_name}: ✗ {row['status']}")
                results.append(row)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='批量计算dddG_elec（支持多进程并行）',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 单进程处理
  python batch_calc_dddg_elec.py ./pdbs/
  
  # 使用4个进程并行
  python batch_calc_dddg_elec.py ./pdbs/ -j 4
  
  # 使用所有CPU核心
  python batch_calc_dddg_elec.py ./pdbs/ -j -1
  
  # 指定输出文件
  python batch_calc_dddg_elec.py ./pdbs/ -o results.csv -j 8
        """
    )
    
    parser.add_argument('pdb_dir', help='PDB文件目录')
    parser.add_argument('-o', '--output', dest='output_csv', 
                        help='输出CSV文件路径（默认为dddg_elec_scores.csv）')
    parser.add_argument('-j', '--jobs', type=int, default=1, dest='n_jobs',
                        help='并行进程数（默认1；-1表示使用所有CPU核心）')
    ...
    batch_calculate_dddg_elec(args.pdb_dir, args.output_csv, args.n_jobs, args.verbose)


if __name__ == "__main__":
    # multiprocessing需要这个保护
